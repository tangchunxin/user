...
class UserListFactory extends ListFactory
{
    ...
    public function __construct($dbobj, $wx_openid = null, $uid = null, $id_multi_str='') 
    {
        //$id_multi_str 是用,分隔的字符串
        if($wx_openid && $uid == null) 
        {
        ...
        elseif($uid && $wx_openid == null) 
        {
            $this->key = $this->key.$uid;
            $this->sql = "select `uid` from `user` where uid='".intval($uid)."'";
            parent::__construct($dbobj, $this->key);
            return true;
        }
        elseif ($id_multi_str) 
